...
def test_joshua_vs_traditional_fixed():
    """修正的对比测试 - 确保数据格式一致性"""
    print("🚀 修正的Joshua方法 vs 传统方法对比测试")
    print("=" * 60)
    ...
    print(f"   - 训练集特征统计:")
    for col in joshua_features_train.columns:
        print(f"     {col}: 均值={joshua_features_train[col].mean():.3f}, 标准差={joshua_features_train[col].std():.3f}")
    ...
    fig.suptitle('Joshua Method vs Traditional Method Comparison (Fixed)', fontsize=16, fontweight='bold')
    ...
    axes[0, 0].set_title('ROC Curves Comparison (Fixed)')
    axes[0, 0].legend()
    ...
    axes[0, 1].set_title('Joshua Method Confusion Matrix (Fixed)')
    ...
    axes[1, 1].bar(x - width/2, joshua_scores, width, label='Joshua (Fixed)', alpha=0.8)
    axes[1, 1].bar(x + width/2, traditional_scores, width, label='Traditional', alpha=0.8)
    axes[1, 1].set_xlabel('Metrics')
    axes[1, 1].set_ylabel('Score')
    axes[1, 1].set_title('Performance Metrics Comparison (Fixed)')
    ...
    plt.tight_layout()
    plt.savefig('joshua_vs_traditional_comparison_fixed.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def main():
    """主函数"""
    print("🚀 修正的Joshua方法测试套件")
    print("=" * 60)
    
    try:
        # 修正的对比测试
        results = test_joshua_vs_traditional_fixed()
